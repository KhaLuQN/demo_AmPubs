...
const NAV = [
  { icon: 'settle', label: 'Chốt số Pub', badge: 3, key: 'settle' },
  { icon: 'payout', label: 'Lệnh chi',    key: 'payout' },
  { icon: 'book',   label: 'Danh bạ Pub', key: 'dir' },
  { icon: 'dash',   label: 'Dashboard',   key: 'dash' },
];
function renderShell(active) {
  const nav = NAV.map(n => `
    <div class="sb-link ${n.key === active ? 'active' : ''}" onclick="stub('Trang &quot;${n.label}&quot; — ngoài phạm vi bản demo này')">
...
function renderSummary() {
  ...
  <div class="summary-band">
    ...
    <div class="sb-cell sb-link" onclick="stub('Danh sách pub đủ điều kiện chi — mở Danh bạ / Lệnh chi')">
      <div class="sb-lbl">💸 Pub đủ điều kiện chi</div>
      <div class="sb-val">4 <span style="font-size:14px">pub</span></div>
      <div class="sb-sub">balance ≥ $100 · xem ▸</div>
    </div>
  </div>`;
}
...
function renderTable() {
  ...
  const body = rows.map(r => {
    ...
    const menu = clickable
      ...
      : `<button class="row-menu" title="Xem sổ pub" onclick="event.stopPropagation();stub('Mở Drawer Pub: balance + sổ tích lũy + trừ tiền + tạo lệnh chi')">⋯</button>`;

    return `
    <tr class="${clickable ? 'row-act' : ''} ${r.state === 'cancel' ? 'row-cancel' : ''}" ${onclick}>
      <td class="col-pub"><span class="c-pub">${r.pub}</span></td>
...
function boot(opts) {
  ...
  renderChips();
  renderTable();
  if (opts && opts.openDrawer) openDrawer(opts.openDrawer);
}
